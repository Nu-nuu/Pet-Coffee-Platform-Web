...
export default function ShopDashboardDetail() {
    ...
    const shopMonthIncome = useSelector(shopMonthIncomeSelector);
    const shopMonthOutcome = useSelector(shopMonthOutcomeSelector);
    ...
    useEffect(() => {
        ...
        if (dateRange.from && dateRange.to) {
            if (checkIncome) {
                dispatch(getShopMonthIncomeThunk(dateRange))
                    .unwrap()
                    .then((res) => {
                ...
                dispatch(getShopMonthOutcomeThunk(dateRange))
                    .unwrap()
                    .then((res) => {
                        setData(ConvertDashboardArray(res?.transactions || []));
                        setCheckHaveTrans(res.transactions !== null);
    ...
    const incomeArr = shopMonthIncome?.transactions;
    const outcomeArr = shopMonthOutcome?.transactions;

    return (
        <div className="shopDashboardDetail">
            <Box m="20px">
                <Header
                        ...
                        <Box display="flex" alignItems="center" gap="3em">
                            ...
                            >
                                ...
                                >
                                    {checkIncome
                                        ? Math.floor(
                                              shopMonthIncome?.balance
                                          )?.toLocaleString()
                                        : Math.floor(
                                              shopMonthOutcome?.balance
                                          )?.toLocaleString()}
                                    <img
                                        src={Coin}
                                        alt=""
                                ...
                                >
                                    ...
                                    <div>
                                        {checkIncome
                                            ? shopMonthIncome?.percent?.toFixed(
                                                  2
                                              )
                                            : shopMonthOutcome?.percent?.toFixed(
                                                  2
                                              )}
                                        {"% "}
                                        <span className="span">
